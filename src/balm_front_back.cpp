...
void cut_voxel(unordered_map<VOXEL_LOC, OCTO_TREE *> &feat_map, pcl::PointCloud<PointType>::Ptr pl_feat,
...
int main(int argc, char **argv)
{
    ...
    while (n.ok())
    {
        ...
        if (plcount > accumulate_window)
        {
            ...
            // The new scan2map method needs several scans to initialize for
            // Velodyne lidar
            ...
            for (int itercount = 0; itercount < 2; itercount++)
            {
                ...
                else
                {
                    ...
                    for (uint i = 0; i < a_size; i++)
                    {
                        ...
                        for (int j = 0; j < ns; j++)
                        {
                            // The point in "pl_surf_centor_map" is defined
                            // below.
        ...
        // For multiple thread, if computer runs slow, the memory may be out of
        // range. 10 is decided in "cut_voxel"
        ...
        // Points in new scan have been distributed in corresponding root node
        // voxel Then continue to cut the root voxel until right size
        for (auto iter = surf_map.begin(); iter != surf_map.end(); ++iter)
        {
            ...
            pl_surf_centor_map += iter->second->root_centors;
            // You can add some distance restrictions in case that
            // pl_surf_centor_map is too large. You can also push points in root
            // voxel into kdtree (loam mapping) You can use
            // "surf_map.erase(iter++)" to erase voxel for saving memory
        }
        ...
        if (plcount >= window_base + window_size && opt_lsv.read_refine_state() == 0)
        {
            ...
            else
            {
                ...
                // We do not recommend use multiple thread on computer with poor
                // performance

                // multithreading
                // map_refine_thread = new thread(&LM_SLWD_VOXEL::damping_iter,
                // &opt_lsv); map_refine_thread->detach();
